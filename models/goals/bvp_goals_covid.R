...
for(i in 1:nrow(league_info)) {
  ...
  ### Fit Model
  model <- stan(file = here('stan/bvp_goals_covid.stan'), 
                data = stan_data, 
                seed = 73097,
                chains = 3, 
                iter = 7000, 
  ...
  write_rds(model, here(paste0('model_objects/bvp_goals_covid/', gsub("\\s", "_", tolower(league)), '.rds')))
  posterior <- extract(model)
  write_rds(posterior, here(paste0('posteriors/bvp_goals_covid/', gsub("\\s", "_", tolower(league)), '.rds')))
}
