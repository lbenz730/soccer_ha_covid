...
for(i in 1:nrow(league_info)) {
  ...
  df <- read_leage_csvs(league) %>% 
    filter(!is.na(home_red_cards), !is.na(away_red_cards))
  ...
  df <- 
    select(df, home, away, home_red_cards, away_red_cards, season, date) %>% 
  ...
  stan_data <- list(
    ...
    h_rc = df$home_red_cards,
    a_rc = df$away_red_cards
  )
  
  ### Fit Model
  model <- stan(file = here('stan/cards/bvp_rc.stan'), 
                data = stan_data, 
                seed = 73097,
                chains = 3, 
                iter = 7000, 
  ...
  write_rds(model, here(paste0('model_objects/bvp_rc/', gsub("\\s", "_", tolower(league)), '.rds')))
  posterior <- extract(model)
  write_rds(posterior, here(paste0('posteriors/bvp_rc/', gsub("\\s", "_", tolower(league)), '.rds')))
}
