...
directory <- 'bvp_yc_lambda3_small'
...
for(i in 1:nrow(league_info)) {
  ...
  stan_data <- list(
    ...
    mu_hf_pre = mean(empirical_baselines$yc_ha_pre),
    mu_hf_post = mean(empirical_baselines$yc_ha_post),
    sd_hf_pre = 3 * sd(empirical_baselines$yc_ha_pre),
    sd_hf_post = 3 * sd(empirical_baselines$yc_ha_post)
  )
