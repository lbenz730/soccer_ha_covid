...
### Read in Posterior HA Draws (Small Version)
posterior_means <- 
  map_dfr(league_info$alias, ~{
    league_ <- gsub("\\s", "_", tolower(.x))
    goals_posterior <- try(suppressWarnings(read_rds(here(glue('posteriors/bvp_goals_no_corr_small/{league_}.rds')))))
    yc_posterior <- try(suppressWarnings(read_rds(here(glue('posteriors/bvp_yc_no_corr_small/{league_}.rds')))))
...
write_csv(posterior_means, here('models/empirical_baselines_small.csv'))
