...
ggsave(here('paper_figures/figures/goals_posterior_means.png'), width = 16/1.2, height = 9/1.2)
...
ggplot(probs, aes(x = p_decrease, y = fct_reorder(league, p_decrease))) +
  ...
  labs(x = 'P(HFA Post-COVID < HFA Pre-COVID)',
       y = 'League',
       title = 'Probability of Decline in Home Field Advantage (Goals)') +
  geom_text(aes(label = paste0(sprintf('%0.1f', 100*p_decrease), '%')), nudge_x = 0.035) +
  scale_x_continuous(labels = scales::percent)
ggsave(here('paper_figures/figures/p_hfa_decline_goals.png'), width = 16/1.2, height = 9/1.2)
