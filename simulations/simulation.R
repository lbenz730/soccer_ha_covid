...
df_summary <-
  df_results %>%
  group_by(model_type, method, team_strength_correlation) %>%
...
ggplot(df_results, aes(x = (estimated_ha_bias), y = model_type, fill = model_type)) +
  facet_grid(~paste('Team Strength Correlation:', team_strength_correlation) ~ paste('DGP:', method)) +
