...
ggsave(here('eda/figures/bvp_yc_covid_1.png'), width = 16/1.2, height = 9/1.2)
...
ggplot(draws, aes(x = posterior_draw)) +
  ...
  labs(x = 'Home Field Advantage Coefficient (Log Scale)',
       ...
       title = 'Home Field Advantage for Selected European Leagues',
       subtitle = 'Bivariate Poisson Model: Yellow Cards')
ggsave(here('eda/figures/bvp_yc_covid_2.png'), width = 16/1.2, height = 9/1.2)   
...
ggsave(here('eda/figures/bvp_goals_yc_means.png'), width = 16/1.2, height = 9/1.2)
...
ggplot(probs, aes(x = p_decrease, y = fct_reorder(league, p_decrease))) +
  ...
  labs(x = 'P(HFA Post-COVID < HFA Pre-COVID)',
       y = 'League',
       title = 'Probability of Decline in Home Field Advantage: Yellow Cards') +
