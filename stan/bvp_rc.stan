data {
  ...
  int<lower=0> h_rc[num_games];                               // red cards for game g
  int<lower=0> a_rc[num_games];                               // away red cards for game g
}
parameters {
...
model {
  ...
  h_rc ~ poisson(lambda1 + lambda3);
  a_rc ~ poisson(lambda2 + lambda3);
}
